...
class EndToEndTest : FunSpec({
    ...
    val executable = if (System.getProperty("os.name").lowercase().contains("win")) {
        projectRoot.resolve("build/install/cli/bin/cli.bat")
    } else {
        projectRoot.resolve("build/install/cli/bin/cli")
    }
    ...
    if (!executable.exists()) {
        System.err.println("Skipping EndToEndTest because executable was not found at ${executable.absolutePath}. Run 'installDist' to enable these tests.")
    }
