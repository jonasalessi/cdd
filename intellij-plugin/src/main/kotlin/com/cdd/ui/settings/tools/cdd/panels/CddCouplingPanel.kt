...
class CddCouplingPanel {
    ...
    init {
        ...
        couplingPanel.border = JBUI.Borders.emptyTop(10)

        panel = couplingPanel
    }

    private fun addPackage() {
        val input = JOptionPane.showInputDialog("Enter package prefix (e.g., com.mycompany):")
    ...
    private fun editSelectedPackage() {
        ...
        if (selectedIndex >= 0) {
            ...
            val input = JOptionPane.showInputDialog("Edit package prefix:", current)
            if (!input.isNullOrBlank()) {
